...
$Header: //commercial/MPSS.TA.2.1.c1/Main/modem_proc/uim/pbm/src/pbm.c#1 $
...
05/12/15   stv     Deprecate usage of TASK_HANDLE
04/07/15   stv     Migrate some efs access to uim_common_efs API
                                     ...
                                     pbm_ecc_mode_type emergency_mode)
{
   ...
   if (data_length > 0)
   {
      UIM_MSG_HIGH_3("Adding ECC num, type 0x%x len %d emergency_mode %d", field_id,
                   data_length, emergency_mode);
      UIM_MSG_HIGH_3("Adding ECC num, 0x%x 0x%x 0x%x",
      ...
      UIM_MSG_HIGH_3("Adding ECC num, cat 0x%x valid %d %d", usim_ecc_serv_cat,
                                        ...
                                        field_length))
      {
         ...
         UIM_MSG_HIGH_0("Could not allocate fields");
         return PBM_ERROR;
      }
      record->unique_id = 0;
...
)
{
  ...
  UIM_MSG_HIGH_2("Reading NV ECC from 0x%x efs_status 0x%x",
                 read_ecc_from, efs_status);
      ...
      read_ecc_from == PBM_READ_ECC_FROM_DIAL_I )
  {
    ...
    for ( i=0; i < PBM_NV_EMERGENCY_NUMBERS; i++ )
    {
      ...
      if (dial_nv.num_digits > 0 && dial_nv.status == NV_EMERGENCY_SD_NUM)
      {
        ...
        {
          /* gives the number of ECC records built through the NV item */
          num_of_nv_ecc++;
        }
...
static void pbm_init_create_conf_file(void)
{
  ...
  char           *conf_files[]                      =
  {
    ...
    UIM_COMMON_EFS_PATH_PBM_NV_ECC_SOURCE_CONFIG,
...
)
{
  ...
  if (efs_status != UIM_COMMON_EFS_SUCCESS)
  {
    UIM_MSG_ERR_1("NV read failed for lte_3gpp_release_ver item status 0x%x",efs_status);
    pbm_3gpp_rel_ver  = LTE_3GPP_REL11 ;
  }
  else
  {
    UIM_MSG_HIGH_1(" pbm_rel_val_read_from_nv from nv is 0x%x", pbm_3gpp_rel_ver);
  }
...
pbm_return_type pbm_list_backup(pbm_record_id_type rec_id)
{
   ...
   if (!cache->pb_cache_array[location])
   {
      ...
      if (!dummy)
      {
         UIM_MSG_ERR_1("Unable to allocate dummy record for 0x%x", rec_id);
         PBM_MEM_FREEIF(node);
         return PBM_ERROR_MEM_FULL;
      }
...
void pbm_handle_cm_ss_evt (pbm_cm_ss_evt_type* cm_ss_evt_ptr)
{
   ...
   switch (cm_ss_evt_ptr->ss_event)
   {
      case CM_SS_EVENT_EMERG_NUM_LIST:
      {
         ...
         PBM_CHECK_PTR_RET(ecc_cache,VOID);
         PBM_CHECK_PTR_RET(ecc_cache->pb_cache_array,VOID);
...
void pbm_wms_cfg_cb (pbm_wms_resp_s_type* wms_resp_ptr)
{
  ...
  UIM_MSG_HIGH_3("PBM Receieved WMS session %d TxId %d PBM Last TxId %d",
	  ...
	  pbm_session_info[pbm_session].last_txid == wms_resp_ptr->info_ptr.t_id)
  {
    ...
    UIM_MSG_HIGH_3("PBM Receieved WMS Ready Event:pbm_wms_init_finished %d pbm_session_subs_ready %d pbm_session %d",
    ...
    {
      UIM_MSG_HIGH_3("PBM session %d in_use_adn_pb %d pbm_session_subs_ready %d",
  ...
  {
    UIM_MSG_HIGH_1("PBM: Session or TxId mismatch for WMS Ready pbm_session %d",pbm_session);
  }
}
